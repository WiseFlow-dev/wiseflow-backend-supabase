...
serve(async (req) => {
  try {
    ...
    if (!cronSecret || requestSecret !== cronSecret) {
      log('recurring_budgets_cron.unauthorized', { 
        has_env_secret: !!cronSecret,
        has_request_secret: !!requestSecret 
      })
      return new Response(
    ...
    log('recurring_budgets_cron.request', { today })

    // Query expired recurring budgets across all users
    ...
    if (queryError) {
      log('recurring_budgets_cron.query_error', { error: queryError.message })
      throw queryError
    }

    if (!expiredBudgets || expiredBudgets.length === 0) {
      log('recurring_budgets_cron.no_expired', { today })
      return new Response(
    ...
    log('recurring_budgets_cron.found_expired', { count: expiredBudgets.length })
    ...
    for (const budget of expiredBudgets) {
      try {
        ...
        if (existingErr) {
          log('recurring_budgets_cron.existing_check_error', { 
            budget_id: budget.id, 
        ...
        if (existing) {
          ...
          log('recurring_budgets_cron.skip_duplicate', { 
            server_key: serverKey, 
          ...
          if (updateError) {
            log('recurring_budgets_cron.update_error', { 
              budget_id: budget.id, 
        ...
        if (insertError) {
          log('recurring_budgets_cron.insert_error', { 
            budget_id: budget.id, 
            server_key: serverKey,
        ...
        createdCount++
        log('recurring_budgets_cron.created', { 
          old_budget_id: budget.id,
          user_id: budget.user_id,
          name: budget.name,
          start_date: startDate,
        ...
        log('recurring_budgets_cron.process_error', { 
          budget_id: budget.id, 
          error: errorMsg 
        })
    ...
    log('recurring_budgets_cron.complete', { 
    ...
    log('recurring_budgets_cron.error', { error: errorMsg })
