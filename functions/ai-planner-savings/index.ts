/**
 * WiseFlow AI Planner - Savings Plan
 * Phase 2: Isolated savings planning function
 *
...
const CURRENT_CYCLE_CUTOFF_DAYS = 10
...
// Types
interface SavingsPlannerRequest {
...
interface SavingsPlannerDependencies {
  supabase: any
  userId: string
}
...
async function resolveMainCurrencySafe(
  ...
  try {
    ...
    log('savings_planner_currency_fallback', {
      userId,
...
export async function handleSavingsPlannerRequest(
  body: SavingsPlannerRequest,
  deps: SavingsPlannerDependencies
) {
  const { supabase, userId } = deps
  const targetSavingsCents = Math.max(0, Math.round(asNumber(body?.targetSavingsCents)))
  const targetMonth = typeof body?.targetMonth === 'string' ? body.targetMonth.trim() : ''
  
  log('savings_planner_request', {
    userId,
    targetSavingsCents,
    targetMonth
  })
  ...
  // Validate inputs
  if (targetSavingsCents <= 0) {
    return {
      type: 'clarifying_question',
      question: 'How much do you want to save in total?'
    }
  }
  ...
  if (!targetMonth) {
    return {
      type: 'clarifying_question',
      question: 'By which month do you want to reach this savings target? (e.g., "2026-12")'
    }
  }

  const targetMonthValidation = validateTargetMonth(targetMonth)
  if (!targetMonthValidation.valid) {
    return {
      type: 'error',
      error: targetMonthValidation.error
    }
  }

  const now = new Date()
  const rawMonthsToSave =
    (targetMonthValidation.year - now.getFullYear()) * 12 +
    ((targetMonthValidation.month - 1) - now.getMonth())
  ...
  const nextCycleStartDate = new Date(cycleEndDate)
  nextCycleStartDate.setDate(nextCycleStartDate.getDate() + 1)
  const nextCycleEndDate = getCycleEndDateFromStart(nextCycleStartDate)

  const planningMode: 'current_cycle' | 'next_cycle' =
    rawElapsedDays <= CURRENT_CYCLE_CUTOFF_DAYS ? 'current_cycle' : 'next_cycle'
  const planningStartDate = planningMode === 'current_cycle' ? cycleStartDate : nextCycleStartDate
  const planningEndDate = planningMode === 'current_cycle' ? cycleEndDate : nextCycleEndDate
  const planningModeLabel = planningMode === 'current_cycle' ? 'Current Cycle' : 'Next Cycle (Projected)'
  const planningCycleTotalDays = Math.ceil((planningEndDate.getTime() - planningStartDate.getTime()) / (1000 * 60 * 60 * 24)) + 1
  ...
  if (planningMode === 'next_cycle') {
    warnings.push(
      `Showing projected next-cycle plan because you are on day ${rawElapsedDays} of this cycle (cutoff day ${CURRENT_CYCLE_CUTOFF_DAYS}).`
    )
  }
  ...
  if (obligationSamples.length === 0) {
    warnings.push('No obligations sample returned. Fixed obligations defaulted to 0 for this run.')
  }
  
  // Fetch transactions from wallet_transactions.
  // Extend the look-back so prior income cycles are available for the no-income fallback below.
  ...
  const windowStart = incomeWindowStart < baseWindowStart ? incomeWindowStart : baseWindowStart
  ...
  if (normalizedTransactions.length > 0) {
    ...
    log('savings_planner_currency_normalization', {
      userId,
      currencyCode,
      ...normalized.metrics,
    })
  ...
  const currentCycleIncomeRawCents = sumIncomeCentsInRange(allTransactions, cycleStartDate, cycleEndDate)
  let incomeCentsForPlanning = currentCycleIncomeRawCents
  let incomeCyclesUsed = 1
  let incomeWindowAveraged = false

  // For longer-term plans, average current and recent positive-income cycles so a paycheck that
  // has not landed yet in this cycle does not zero out the plan. Cycles with no income are skipped
  // so they don't drag the average down.
  if (useAverageIncome) {
    const incomeSamples: number[] = []
    if (currentCycleIncomeRawCents > 0) incomeSamples.push(currentCycleIncomeRawCents)
  ...
  // Savings planning should not be driven by one tiny current-cycle sample.
  ...
  // This mirrors the Emergency Fund and Vacation planners: previous cycle is trusted more
  // for long-term planning, while current-cycle-only categories are ignored during week one.
  ...
  const budgetLocks = buildBudgetLockContext({
    budgets: budgetRows,
    windowStartISO: toDateISO(planningStartDate),
    windowEndISO: toDateISO(planningEndDate),
    categoryNameById
  })
  ...
  log('savings_planner_budget_locks', {
    userId,
  ...
  // Monthly amount needed to hit the selected target month.
  const requiredMonthlyCents = Math.ceil(targetSavingsCents / monthsToSave)
  const adjustedRequiredCents = requiredMonthlyCents
  ...
  log('savings_planner_category_filtering', {
    userId,
  ...
  // For savings plans, cuts are needed when there is a gap.
  // If there is no gap, cuts are optional buffer only.
  const cutTargetCents = gapCents > 0 ? gapCents : adjustedRequiredCents
  ...
  const verdict = computeVerdict(baseCapacityCents, adjustedRequiredCents, maxCoveragePct)
  
  const cutMode = gapCents > 0 ? 'needed_to_hit_target' : 'optional_buffer'
  
  const savingsProjection = computeSavingsProjectionFields(
  ...
  if (verdict === 'yes' && gapCents === 0) {
    wiseyNote = `You can hit this savings target by ${targetMonth} from your projected monthly flow.`
  } else if (verdict === 'yes') {
    wiseyNote = `You can hit this target by ${targetMonth} by trimming about ${formatCurrencyTextFromCents(selectedCutCents, currencyCode)}/month from optional categories.`
  } else if (verdict === 'close') {
    wiseyNote = `You're close for the ${targetMonth} target. With disciplined cuts, you can cover most of it.`
  } else {
    wiseyNote = `This target is hard to reach by ${targetMonth}. Consider lowering the target or extending the timeline.`
  }
  ...
  // Log calculation
  log('savings_planner_calculation', {
    userId,
    goalAmountCents: adjustedRequiredCents,
    currentCycleIncomeRawCents,
  ...
  return {
    type: 'savings_plan',
    goalAmountCents: targetSavingsCents,
    targetMonth,
    monthsToSave,
    requiredMonthlySavingCents: adjustedRequiredCents,
    realisticAmountCents: savingsProjection.realisticAmountCents,
    verdict,
    monthlySurplusCents: baseCapacityCents,
    recurringMonthlyCapacityCents: savingsProjection.recurringMonthlyCapacityCents,
    recurringSavingsRatePct: savingsProjection.recurringSavingsRatePct,
    isOneTimeAchievable: savingsProjection.isOneTimeAchievable,
    fixedObligationsCents,
    trimCategories,
    totalTrimmableCents,
    selectedCutCents,
    maxOptionalCutCapacityCents: totalTrimmableCents,
    baseCoveragePct,
    comfortCoveragePct,
    maxCoveragePct,
    cutMode,
    requiredCutToTargetCents: gapCents,
    savingsRatePct: savingsProjection.savingsRatePct,
    dailyDeductionCents: Math.max(1, Math.round(adjustedRequiredCents / Math.max(1, planningCycleTotalDays))),
    projections: savingsProjection.projections,
    wiseyNote,
    currencyCode,
    ...
    calculationContext: {
      mode: planningMode,
      modeLabel: planningModeLabel,
      incomeWindow: {
        label: incomeWindowAveraged
          ? `Average of ${incomeCyclesUsed} recent cycle(s)`
          : (planningMode === 'current_cycle' ? 'Current Cycle' : 'Current Cycle (Projection Basis)'),
      ...
      variableSpendWindow: {
        label: planningMode === 'current_cycle' ? 'Current Cycle' : 'Current Cycle (Projection Basis)',
    ...
    dataQuality,
    cyclesUsed: planningMode === 'current_cycle' ? 'current' : 'next',
...
function validateTargetMonth(targetMonth: string): TargetMonthValidation {
  ...
  if (!match) {
    return { valid: false, error: 'Target month must be in YYYY-MM format (e.g., "2026-12")' }
  }
...
serve(async (req) => {
  ...
  try {
    ...
    const result = await handleSavingsPlannerRequest(body, { supabase: userScopedSupabase, userId })
    ...
    logError('savings_planner_error', {
      error: error.message,
      stack: error.stack
    })
