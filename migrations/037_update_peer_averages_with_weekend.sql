-- 037_update_peer_averages_with_weekend.sql
-- Update get_peer_averages_by_bracket to include weekend_ratio
...
RETURNS TABLE (
  ...
  avg_spending_ratio DECIMAL,
  avg_weekend_ratio DECIMAL,
  peer_count INT
) AS $$
BEGIN
  -- Aggregate from user_monthly_scores
  ...
  SELECT 
    ...
    AVG(ums.spending_ratio)::DECIMAL as avg_spending_ratio,
    AVG(ums.weekend_ratio)::DECIMAL as avg_weekend_ratio,
    COUNT(DISTINCT ums.user_id)::INT as peer_count
  FROM public.user_monthly_scores ums
  WHERE ums.month = p_month
    AND (ums.savings_rate_v2 IS NOT NULL 
         OR ums.spending_ratio IS NOT NULL 
         OR ums.weekend_ratio IS NOT NULL);
END;
