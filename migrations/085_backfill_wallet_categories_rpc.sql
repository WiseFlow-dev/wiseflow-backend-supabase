-- Backfill wallet_transactions categories from txn_categorization after delayed inserts.
-- This handles mirror misses when AI finishes before wallet_transactions rows exist.
...
BEGIN
  ...
  WITH matched AS (
    ...
    JOIN public.transactions t
     ...
     AND t.name = wt.title
     AND wt.date >= (t.txn_date::date)::timestamptz
     AND wt.date < ((t.txn_date::date + interval '1 day'))::timestamptz
